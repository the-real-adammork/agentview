...
function App() {
  ...
  // otherwise the repo of whatever session is currently selected. Always
  // informative — only the Repos browser itself shows the plain "REPOS" label.
  const headerRepo = repoFilter || (session ? getRootSession(session).cwd : null);
  ...
  return (
    <div className="shell" data-screen-label={`Observatory · ${view}`}>
      ...
      <header className="header">
        ...
        <SessionSquare session={session} onClick={() => setView("sessions")} active={view === "sessions"} railStart={view !== "repos"} />

        <nav className="nav">
          {VIEWS.map((v, i) => {
            const activeIdx = VIEWS.findIndex((x) => x.key === view);
            const inRail = activeIdx >= 0 && i <= activeIdx;
            return (
              <button key={v.key} data-active={view === v.key} data-rail={inRail ? "on" : undefined} onClick={() => setView(v.key)}>
...
function SessionSquare({ session, onClick, active, railStart }) {
...
/* ============================================================
   ...
   ============================================================ */
function SessionsView({ selected, setSelected, setView, repoFilter, setRepoFilter, onTokenBump }) {
...
function TimelineView({ session, setSelected, setView, onTokenBump }) {
  ...
  const allEvents = useMemo(() => [...baseEvents, ...liveEvents], [baseEvents, liveEvents]);
    ...
    <div className="timeline">
      ...
      <section className="tl-main">
        ...
        <div className="tl-stream" ref={streamRef}>
          {(() => {
            // compute deltas in chronological order, then reverse so newest is on top
            let prevTotal = 0;
            const rows = filtered.map((e, i) => {
              const key = e._id || `${e.ts.getTime()}-${i}`;
              const isOpen = expanded.has(key);
              let delta;
              if (e.kind === "token_count") { delta = e.total - prevTotal; prevTotal = e.total; }
              return (
                <div key={key} className={"ev-wrap" + (e._new ? " ev-enter" : "")}>
                  <EventRow e={e} delta={delta} outByCall={outByCall} isOpen={isOpen} toggle={() => toggle(key)} onJump={(id) => { setSelected(id); }} />
                </div>
              );
            });
            return rows.reverse();
