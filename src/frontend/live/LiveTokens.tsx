...
export function LiveTokenTotal({ fallback }: { fallback: number }) {
  ...
  return <span data-testid="live-token-total">{compactNumberFormatter.format(total)}</span>;
}

export function LiveSessionTokens({ sessionId, fallback }: { sessionId: string; fallback: number }) {
